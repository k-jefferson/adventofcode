...
export const day3 = (wire1: Array<string>, wire2: Array<string>): number => {
    const path1 = computeWirePath(wire1);
    const path2 = computeWirePath(wire2);

    const crossingPoints = findCrossingPoints(path1, path2);
    return findClosestCrossingPoint(crossingPoints);
};

const computeWirePath = (wire: Array<string>): any => {
    let x = 0;
    let y = 0;
    const positions: any = {};
    
    wire.forEach(p => {
        const direction = <Direction>p[0];
        const move = +p.substr(1);
        let moveFunction: (() => void) = () => {};
...
const findCrossingPoints = (path1: any, path2: any): Array<string> => {
    const path1Keys = Object.keys(path1);
    const path2Keys = Object.keys(path2);

    const path1KeysLength = path1Keys.length;
    const path2KeysLength = path2Keys.length;

    const findPoints = (a: Array<string>, b: any): Array<string> => a.filter(x => b[x] !== undefined);

    return path1KeysLength < path2KeysLength ? findPoints(path1Keys, path2) : findPoints(path2Keys, path1);
};

const findClosestCrossingPoint = (crossingPoints: Array<string>): number => {
    const manhattanDistances: Array<number> = []; 
    crossingPoints.forEach(p => {
        const split = p.split(':');
        manhattanDistances.push(Math.abs(+split[0]) + Math.abs(+split[1]));
    });

    const manhattanLength = manhattanDistances.length;
    let min = manhattanDistances[0];
    for (let i = 1; i < manhattanLength; i++) {
        if (manhattanDistances[i] < min) {
            min = manhattanDistances[i];
        }
    }
